...
2.  **Build and Run with Docker Compose:**
    ...
    ```
    This will start the service defined in `docker-compose.yml`. By default, it's configured to run the `process` command on the `/media` directory.

3.  **Accessing the CLI in a Running Container:**
    If you want to run ad-hoc commands or interact with the CLI directly:
    ```bash
    docker-compose exec unified-media-processor bash
    ```
    (Note: `unified-media-processor` should match the `container_name` in your `docker-compose.yml`. The original `docker-compose.yml` used `simple-media-scanner`, this should be updated for consistency if desired.)
...
Assuming your `docker-compose.yml` is configured (see `docker-compose.yml.example` or the one provided in the repo):

1.  **Place your media files** in the directory you've mapped to `/media` in the container (e.g., `./media/` on your host).
2.  **Run the processing pipeline:**
    ```bash
    docker-compose run --rm unified-media-processor process /media --output-path /app/output --log-path /app/logs
    ```
    (Adjust `unified-media-processor` if you've named your service differently in `docker-compose.yml`.)
...
3.  **To run other commands,** simply change the command part:
    ```bash
    # Example: Scan only
    docker-compose run --rm unified-media-processor scan /media
    ...
    docker-compose run --rm unified-media-processor report-errors /app/logs/scan_log.json
    ```
